...
/**
 * This build's version, sent with every setup test.
 *
 * `@notibase/web` was the only SDK with no version constant anywhere in
 * it: no user agent, no `sdk_version` in its setup-test report, nothing.
 * It was invisible to version telemetry, and — because there was no
 * string to compare — outside what `scripts/check-sdk-versions.mjs`
 * could watch, so "which build is this customer on" had no answer at
 * all for the SDK that runs in the most places.
 *
 * Kept in step with package.json and the changelog by that script.
 */
...
export class Notibase {
  ...
  /**
   * Sign this browser out: it is no longer that person's.
   *
   * The inverse of `identify`, and the call for a sign-out button. The
   * browser stays subscribed and push keeps working — permission is still
   * granted and the subscription is still live. It simply stops belonging
   ...
   * a shared computer. The next `identify` attaches it again.
   *
   * **Not `unsubscribe`.** That records a durable opt-out against the push
   * subscription and tears the subscription down; it is lifted only from
   * your own backend with a server key. Reaching for it on sign-out
   * silences the browser for whoever signs in next, permanently, with
   * nothing on the page able to undo it.
   *
   * Triggers are cleared too: a trigger is a fact about the person in front
   * of you and none of them survive them leaving. What is kept is how often
   * each in-app message has been shown, which is a property of the browser
   * rather than of anyone signed into it.
   *
   * Never throws — a sign-out control must not fail because the network did.
   */
  async logout(): Promise<boolean> {
    this.triggers.clear();
    if (!this.deviceId) return true;     // never subscribed, nothing to detach
    try {
      await this.api("/v1/logout", { device_id: this.deviceId });
    } catch {
      return false;
    }
    return true;
